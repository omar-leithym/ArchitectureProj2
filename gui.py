...
# In GUI code, update_timeline_table function:
def update_timeline_table(timeline_data):
    ...
    for instr in timeline_data:
        # Create values tuple with status
        values = (
            instr['instruction'],
            ...
            "Flushed" if instr.get('flushed', False) else "Completed"
        )
        
        # Use different tag for flushed instructions (for coloring)
        tag = "flushed" if instr.get('flushed', False) else ""
        timeline_table.insert("", "end", values=values, tags=(tag,))
    
    # Configure tag colors
    timeline_table.tag_configure("flushed", background="#ffcccc")
