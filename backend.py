...
def execute_single_step():
    ...
    step_pending = True
    #clear_output_cycles()
    execution_unit.get_instruction_timeline()


def clear_output_cycles():
    global registers, program_counter, memory, labels, executable_instructions
    global output_to_gui_global, stop_simulation
...
def main(instructions_text, memory_text, output_to_gui, starting_pc, single_step=False):
   ...
   # if not hasattr(main, "initialized"):
    ...
    step_pending = not single_step
    reg_manager.__init__()
    execution_unit.__init__()
    ...
    while not stop_simulation and running and 0 <= (program_counter - base_address) < instruction_count:
        ...
        if step_mode:
            if not step_pending:
                output_to_gui_global(f"Waiting for next step at cycle {execution_unit.current_cycle}")
                break
            step_pending = False
        ...
        # Calculate the index in the executable_instructions list
            ...
            if index >= len(executable_instructions):
                break
            ...
            instruction_record = { 
                ...
                'src_regs': [f"r{rB}", f"r{rC}"] if (rB and rC) else [f"r{rB}"] if rB else [],
                'offset': imm_or_label if imm_or_label else None
            }
            execution_unit.issue_instruction(instruction_record, reg_manager)
            execution_unit.execute_process(reg_manager)
            output_to_gui_globalRegisters(instruction = f"CYCLE {execution_unit.current_cycle}")
            issued_success = execution_unit.issue_instruction(instruction_record, reg_manager)
            if not issued_success:
                output_to_gui_global(f"Issue failed at PC={program_counter}: {opcode}")
            else:
                w_len = len(execution_unit.waiting_to_execute_instructions)
                output_to_gui_global(f"Issued {opcode}; Waiting queue now length {w_len}")
                execution_unit.execute_process(reg_manager)
