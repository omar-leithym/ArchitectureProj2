...
def increment_pc():
    global pc_value
    pc_value += 2  # Increment by 2 for 16-bit words
    return pc_value
...
def beq(rA, rB, offset):
    ...
    if registers[rA] == registers[rB]:
        old_pc = pc_value
        pc_value = pc_value + 2 + offset # Increment by 2 for 16-bit words and offset
        output_to_gui_global(f"BEQ: Branch taken to PC+2+offset*2 = {pc_value}, r{rA}={registers[rA]}, r{rB}={registers[rB]}")
        return True
    else:
...
def call(label):
    ...
    if label in labels:
        registers[1] = pc_value + 2  # Store return address in R1, increment by 2
...
def main(instructions_text, memory_text, output_to_gui, starting_pc, fu_config):
    ...
    for line in instructions_text.splitlines():
        ...
        if stripped_line.endswith(':'):
            ...
            # Map label to the instruction address - use 2 for 16-bit words
            labels[label_name] = base_address + len(executable_instructions) * 2
        else:
    ...
    if pc_value < base_address or pc_value >= base_address + instruction_count * 2:
    ...
    for index in range(len(executable_instructions)):
        ...
        # In backend.py, in the main function:
        if opcode == 'STORE':
            instruction_record = { 
                'op': opcode,
                'dest_reg': None,  # STORE doesn't write to a register
                'src_regs': [f"r{rA}", f"r{rB}"],  # rA = value to store, rB = base register
                'offset': imm_or_label,
                'value_reg': f"r{rA}",  # Register containing value to store
                'base_reg': f"r{rB}"    # Base register for address calculation
            }
        ...
        # Add PC value to instruction record for branch handling
        instruction_record['pc'] = pc_value + index*2
        
        issued_success = execution_unit.issue_instruction(instruction_record, reg_manager)
        print(f"[PC = {pc_value + index*2}] Executing: {line}")  # Update for 16-bit words
        
        if not issued_success:
            output_to_gui_global(f"Issue failed at PC={pc_value + index*2}: {opcode}")  # Update for 16-bit words
        else:
    ...
    while execution_unit.has_pending_instructions():
        ...
        state = execution_unit.get_state()
        output_to_gui_global(f"Cycle {state['cycle']}: {len(state['issued'])} waiting, {len(state['executing'])} executing")
